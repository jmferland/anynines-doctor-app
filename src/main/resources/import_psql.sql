...
INSERT INTO bill(id, merchant_id, customer_id, creationDate, token, descriptor, amount, currency) values( nextval( 'hibernate_sequence') , (SELECT m.id FROM merchant m WHERE m.name = 'COPYandPAY Integration Guide') , (SELECT c.id FROM customer c WHERE c.firstName = 'Juergen') , NOW() , '01234567890123456789012345678901' , 'Misc. Medical Expenses' , 1.00 , 'EUR');
